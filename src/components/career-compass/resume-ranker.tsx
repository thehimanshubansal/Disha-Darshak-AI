...
import { FileUp, CheckCircle, Lightbulb, ThumbsUp, ThumbsDown, Sparkles, Flame, Loader2 } from 'lucide-react';
...
// This component is memoized to prevent re-rendering when parent state changes.
...
export default function ResumeRanker() {
  ...
  // Determine which main view to show in the right panel
  const showResults = !isAnalyzing && !isRoasting && (rankingResult || roastResult);
  const showPreview = pdfPreviewUrl && !showResults;
  const showPlaceholder = !showPreview && !showResults;
                ...
                <CardContent className="space-y-4">
                    ...
                    <div>
                     ...
                     <div>
                        ...
                        <Input id="field" placeholder="e.g., FinTech" value={field} onChange={e => setField(e.target.value)} />
                    </div>
                    <div className="pt-2">
                        <h3 className="text-sm font-medium mb-2">4. Choose Action</h3>
                        <div className="grid grid-cols-2 gap-2">
                            <Button onClick={handleAnalyze} disabled={isAnalyzing || isRoasting || !pdfBase64}>
                                {isAnalyzing && <Loader2 className="animate-spin" />}
                                Rank Resume
                            </Button>
                            <Button onClick={handleRoast} disabled={isAnalyzing || isRoasting || !pdfBase64} variant="outline">
        ...
        <div className="lg:col-span-2 relative">
            ...
            <AnimatePresence mode="wait">
                {showResults ? (
                    ...
                    <MemoizedResumePreview url={pdfPreviewUrl} fileName={uploadedFile?.name} />
                ) : (
