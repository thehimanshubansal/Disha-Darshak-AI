...
export default function SkillAssessmentForm({ onComplete, questions: customQuestions }: SkillAssessmentFormProps) {
    const { user, refreshEvaluations, skillAssessmentState, setSkillAssessmentState, handleClearAssessmentState } = useAppContext();
    ...
    const { answers, analysis, selectedRole, roadmap, isFinished, currentQuestionIndex } = skillAssessmentState;
    ...
    const handleNext = () => {
        if (currentQuestion.required && (answers[currentQuestion.id] === undefined || answers[currentQuestion.id] === '' || (typeof answers[currentQuestion.id] === 'object' && answers[currentQuestion.id]?.selected === 'Other' && !answers[currentQuestion.id]?.text))) {
            setError('Please provide an answer to continue.');
            return;
        }
        if (currentQuestionIndex < questions.length - 1) {
            setSkillAssessmentState({ currentQuestionIndex: currentQuestionIndex + 1 });
        } else {
            handleSubmit();
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setSkillAssessmentState({ analysis: data, isFinished: true });
    ...
    const handleRoleSelectAndGenerateRoadmap = async (role: string) => {
        setSkillAssessmentState({ selectedRole: role });
        ...
        try {
            ...
            if (finalData.roadmap) {
                setSkillAssessmentState({ roadmap: finalData.roadmap });
                if (user) {
    ...
    const renderQuiz = () => (
        <>
            ...
            <div className="p-6 border-t flex justify-between items-center">
                <Button variant="outline" onClick={() => setSkillAssessmentState({ currentQuestionIndex: currentQuestionIndex - 1 })} disabled={currentQuestionIndex === 0}><ArrowLeft className="mr-2 h-4 w-4" /> Previous</Button>
    ...
    const renderResults = () => {
        if (roadmap) {
            return (
                 <motion.div key="roadmap" variants={panelVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                    ...
                    <RoadmapTimeline steps={roadmap.timeline_steps} />
                    
                    <p className="text-center text-sm text-muted-foreground mt-6 italic">{roadmap?.final_advice}</p>
                    <div className="text-center mt-4">
                        <Button onClick={resetForm} variant="outline"><RefreshCw className="mr-2 h-4 w-4" /> Start a New Quiz</Button>
                    </div>
                </motion.div>
            );
        }
