...
export default function VoiceInterviewUI({ messages, loading, onSend, onEnd }: VoiceInterviewUIProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // --- MODIFICATION START ---
      // This handler now correctly accumulates the final transcript
      // without resetting it on each event.
      recognitionRef.current.onresult = (event: any) => {
        let interimTranscript = '';
        // Iterate from the last processed result index.
        for (let i = event.resultIndex; i < event.results.length; ++i) {
          // If the result is final, append it to our persistent final transcript ref.
          if (event.results[i].isFinal) {
            finalTranscriptRef.current += event.results[i][0].transcript + ' ';
          } else {
            // Otherwise, it's an interim result that might change.
            interimTranscript += event.results[i][0].transcript;
          }
        }
        // Update the displayed transcript with the accumulated final transcript
        // and the current interim transcript for real-time feedback.
